...
class Watermark16Sector:
    ...
    def generate_template(self, numbit=None):
        ...
        for num in range(self.n_sectors):
            ...
            #取start_angle-end_angle的1/4的角度
            # start_angle1 = start_angle + (end_angle - start_angle) / 4
            # end_angle1= end_angle - (end_angle - start_angle) / 4
            # if numbit[num] == 1:
            #     r=self.r[0]
            # else:
            #     r=self.r[1]
...
if __name__ == "__main__":
    # 初始化
    wm = Watermark16Sector(L1=512, k1=30000, r=[5,10], r_range=2, n_sectors=4)
    
   
        
    Tm, M1 = wm.generate_template(numbit=[1,0,1,0])
    # cv2.imwrite(f"img_mask/watermark_template.png", Tm)  # 保存水印模板图像以供调试
    # cv2.imwrite(f"img_mask/watermark_spectrum.png", cv2.normalize(M1, None,255,255, cv2.NORM_MINMAX).astype(np.uint8))  # 保存水印频谱图像以供调试
    cv2.imwrite(f"img_mask/template1010.png", Tm)  # 保存水印模板图像以供调试
    cv2.imwrite(f"img_mask/spectrum1010.png", cv2.normalize(M1, None, 0, 255, cv2.NORM_MINMAX).astype(np.uint8))  # 保存水印频谱图像以供调试
