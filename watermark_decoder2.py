...
class AdvancedWatermarkDecoder(nn.Module):
    def __init__(self, n_sectors=60):
        super().__init__()
        
        # A. 空间域预处理 (增强：更深的ResUNetFilter)
        self.pre_filter = ResUNetFilter()  # 保持或可进一步增强
        
        # B. 极坐标后的特征提取 (增强：增加深度和通道数)
        # 输入形状: (B, 1, R_bins, T_bins) -> 例如 (B, 1, 32, 128)
        self.feature_extractor = nn.Sequential(
            nn.Conv2d(1, 128, kernel_size=(5, 5), stride=(1, 2), padding=2),
            nn.GroupNorm(16, 128),
            nn.ReLU(),
            ...
            nn.GroupNorm(64, 512),
            nn.ReLU(),
            nn.AdaptiveAvgPool2d((1, n_sectors))  # 强制压缩到 num_bits个位置
        )
        
        # C. 序列解码 (增强：多层注意力机制)
        # 将 CNN 输出视为长度为 num_bits 的序列
        self.attention1 = nn.MultiheadAttention(embed_dim=512, num_heads=8, batch_first=True)
        self.attention2 = nn.MultiheadAttention(embed_dim=512, num_heads=8, batch_first=True)
    ...
    def forward(self, x, save_debug=False):
        ...
        polar_feat = cartesian_to_polar(mag, output_shape=(32, 360), max_radius=25)
        if save_debug:
        ...
        feat = feat.squeeze(2).permute(0, 2, 1)  # [B, n_sectors, 512]
        
        # 多层注意力机制
        attn_out1, _ = self.attention1(feat, feat, feat)
        # attn_out1 = self.norm1(attn_out1 + feat)  # 残差连接
        # attn_out1 = self.dropout(attn_out1)
        
        # attn_out2, _ = self.attention2(attn_out1, attn_out1, attn_out1)
        # attn_out2 = self.norm2(attn_out2 + attn_out1)  # 残差连接
        # attn_out2 = self.dropout(attn_out2)
        
        logits = self.classifier(attn_out1)
...
class WatermarkDecoder(nn.Module):
    def __init__(self, block_size=512, num_bits=60, r=[25.0, 40.0, 55.0, 70.0], roi_size=15, embed_dim=256, num_heads=8, num_layers=6):
        super().__init__()
        self.num_bits = num_bits
        ...
        self.roi_net = nn.Sequential(
            ...
            nn.Flatten(),
            nn.Linear(256, embed_dim // 2)  # 局部特征维度
        )
        
        # 5. Transformer编码器（增强：增加embed_dim和num_layers）
        self.pos_embedding = nn.Parameter(torch.randn(1, num_bits, embed_dim))  # 位置嵌入
        encoder_layer = TransformerEncoderLayer(d_model=embed_dim, nhead=num_heads, dim_feedforward=1024, dropout=0.1, batch_first=True)
