"""
v17 无镂空水印训练 - 纯pair噪声版（与v18对比用）

与v18的区别：使用纯二值模板(0/255)，无镂空

用法:
    python train_cb_v17_pair.py --config config/train_cb_v17_pair.yaml
"""
...
from dataset_v17 import WatermarkDatasetV17
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler('train_cb_v17_pair.log')
    ]
)
...
def build_dataset(cfg, transform, noise_level='none', alpha_embed=0.016,
    ...
    if datasets_cfg is not None:
        dataset = WatermarkDatasetV17(datasets=datasets_cfg, **kwargs)
    else:
        dataset = WatermarkDatasetV17(
...
def build_val_dataset(cfg, transform, noise_level='none', alpha_embed=0.016,
    ...
    if val_datasets_cfg is not None:
        dataset = WatermarkDatasetV17(datasets=val_datasets_cfg, **kwargs)
    else:
        dataset = WatermarkDatasetV17(
...
def main():
    parser = argparse.ArgumentParser(description='v17 无镂空水印训练 - 纯pair噪声')
    ...
    # 创建输出目录
    output_dir = cfg.get('output_dir', f'output/v17_pair_{datetime.now().strftime("%Y%m%d_%H%M%S")}')
    ...
    logger.info("=" * 60)
    logger.info("v17 无镂空水印训练 - 纯pair噪声")
    logger.info("=" * 60)
