"""
v11 版本：Inference script for full-image tiled watermark dataset with arbitrary 512x512 crops.

使用 v11 解码器（3环：r=12, r=18, r=25）。

Usage:
  python inference_test_bcgd_wm_full_any_crop_v11.py \
    --gt_json /mnt/lpl/fftmask/watermarked_output_cb_v11/embed_gt_mapping.json \
    --input_dir /mnt/lpl/fftmask/watermarked_output_cb_v11 \
    --output_dir /mnt/lpl/fftmask/decode_cb_v11 \
    --num_crops 16
"""
...
def main():
    ...
    parser.add_argument("--model_path", type=str,
                        default="/mnt/lpl/fftmask/output/finetune_v11/20260723_121008/models/best_cb_decoder_v11.pth")
    ...
    # 支持子文件夹结构：优先使用 watermarked_path，其次在 input_dir 下查找
    image_paths = []
    gt_bits_list = []
    ...
    for img_idx in tqdm(range(len(image_paths)), desc="Decoding"):
        ...
        # Resize shortest edge to 1024 before cropping
        image = resize_min_edge(image, min_size=1024)

        h, w = image.shape[:2]
        vis_img = image.copy()

        # 中心 crop
        center, cy, cx = get_center_crop(image, block_size)
        ...
        # 画中心框（绿色）
        cv2.rectangle(vis_img, (cx, cy), (cx + block_size, cy + block_size), (0, 255, 0), 3)
        ...
        for ci in range(args.num_crops - 1):
            rand_crop, ry, rx = get_random_crop(image, block_size)
            ...
            cv2.rectangle(vis_img, (rx, ry), (rx + block_size, ry + block_size), color, 2)
